...
const handler = async (req: NextApiRequest, res: NextApiResponse<OkPayload | ErrorPayload>) => {
  ...
  if (body.assistant_id) {
    try {
      let threadId = disableThreadReuse ? null : body.thread_id;
      ...
      if (!runRes.ok) {
        return res.status(500).json({ ok: false, error: { message: 'assistant_unavailable', details: 'run failed to start' } });
      }

      const run = await runRes.json();
      if (run.status === 'failed') {
        return res.status(500).json({ ok: false, error: { message: 'assistant_unavailable', details: run.last_error } });
      }
      ...
      if (status !== 'completed') {
        return res.status(500).json({ ok: false, error: { message: 'assistant_unavailable', details: 'run timeout' } });
      }
